...
// TestManager_RealPTYLifecycle exercises the default Starter (session.Start /
// creack/pty) with a trivial command to prove end-to-end state transitions on
// a real handle. Skipped on non-unix platforms and when /bin/echo is missing.
func TestManager_RealPTYLifecycle(t *testing.T) {
	...
	sink := &fakeSink{}
	m := NewManager(sink, nil) // DefaultStarter
	...
	req := StartRequest{
		...
		Cmd:      exec.Command("/bin/echo", "hello-from-mux"),
		BootMode: "",
	}

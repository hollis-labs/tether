...
func TestMintAgentURN_format(t *testing.T) {
	...
	urn, err := mintURN(context.Background(), src, agentIDPrefix, neverCollides)
	if err != nil {
...
func TestMintProjectURN_format(t *testing.T) {
	src := bytes.NewReader(make([]byte, idSuffixLen))
	urn, err := mintURN(context.Background(), src, projectIDPrefix, neverCollides)
	if err != nil {
		t.Fatalf("mintURN: %v", err)
	}
...
func TestMintURN_collisionRetry(t *testing.T) {
	...
	urn, err := mintURN(context.Background(), src, agentIDPrefix, exists)
	if err != nil {
		t.Fatalf("mintURN: %v", err)
	}
	if calls != 2 {
...
func TestMintURN_exhausted(t *testing.T) {
	...
	_, err := mintURN(context.Background(), src, agentIDPrefix, alwaysCollides)
...
func TestMintURN_contextCanceled(t *testing.T) {
	...
	cancel()
	src := bytes.NewReader(make([]byte, idSuffixLen))
	_, err := mintURN(ctx, src, agentIDPrefix, neverCollides)
...
func TestMintURN_exhaustedAtBoundary(t *testing.T) {
	...
	_, err := mintURN(context.Background(), src, agentIDPrefix, collides)
	if !errors.Is(err, ErrMintExhausted) {
		t.Fatalf("err = %v; want ErrMintExhausted", err)
	}

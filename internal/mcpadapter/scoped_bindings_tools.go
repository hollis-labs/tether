...
func (a *Adapter) registerScopedBindingsTools(s *gomcp.Server) {
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			gomcp.StringProp("scope", "Consumer-owned scope, e.g. a run or team id.", true),
			gomcp.StringProp("slot", "Role/slot name within the scope, e.g. 'reviewer'.", true),
			gomcp.StringArrayProp("target_urns", "One or more target URNs for this slot.", true),
			gomcp.StringProp("created_by", "Caller URN recorded as provenance for this revision.", true),
		),
	...
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			gomcp.StringProp("scope", "Consumer-owned scope.", true),
			gomcp.StringProp("slot", "Role/slot name within the scope.", true),
			gomcp.BooleanProp("single", "Resolve to exactly one target (default false: return every target).", false),
		),

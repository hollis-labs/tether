...
func (a *Adapter) registerAITools(s *gomcp.Server) {
	...
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			...
			gomcp.ObjectProp("request", "Full normalized llm.Request object. Mutually exclusive with text and request_json.", false),
			gomcp.StringProp("request_json", "Full normalized llm.Request encoded as JSON. Mutually exclusive with text and request.", false),
			gomcp.StringProp("system_prompt", "Optional system prompt", false),
			gomcp.StringArrayProp("image_urls", "Optional image URLs to append as user content parts", false),
			gomcp.StringProp("image_base64", "Optional inline image bytes as base64 for the shorthand request form", false),
			gomcp.StringProp("image_mime_type", "MIME type for image_base64, e.g. image/png", false),
			gomcp.StringProp("provider", "Configured provider id hint", false),
			gomcp.StringProp("model", "Model hint", false),
			gomcp.StringProp("mode", "Mode hint, such as summarize or tool-heavy", false),
			gomcp.StringProp("intent", "Intent hint", false),
			gomcp.StringProp("request_id", "Optional request correlation id", false),
			gomcp.StringProp("session_id", "Optional session correlation id", false),
			gomcp.StringProp("caller_id", "Optional caller correlation id", false),
			gomcp.NumberProp("max_output_tokens", "Optional max output tokens hint", false),
			gomcp.NumberProp("token_budget", "Optional token budget hint", false),
			gomcp.NumberProp("cost_budget_usd", "Optional cost budget hint in USD", false),
			gomcp.NumberProp("latency_target_ms", "Optional latency target in milliseconds", false),
		),
	...
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			...
			gomcp.StringProp("provider", "Configured provider id hint", false),
			gomcp.StringProp("model", "Model hint", false),
			gomcp.StringProp("mode", "Mode hint", false),
			gomcp.StringProp("intent", "Intent hint", false),
			gomcp.StringProp("request_id", "Optional request correlation id", false),
			gomcp.StringProp("session_id", "Optional session correlation id", false),
			gomcp.StringProp("caller_id", "Optional caller correlation id", false),
	...
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			...
			gomcp.StringProp("session_id", "Filter by session correlation id", false),
			gomcp.StringProp("caller_id", "Filter by caller correlation id", false),
			gomcp.StringProp("operation", "Filter by operation kind, default chat", false),
			gomcp.StringProp("since", "RFC3339 lower-bound timestamp", false),
		),
	...
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			gomcp.StringProp("provider", "Filter by configured provider id", false),
			gomcp.StringProp("model", "Filter by model id", false),
			gomcp.StringProp("session_id", "Session id used for session-scoped budgets", false),
			gomcp.StringProp("caller_id", "Caller id used for caller-scoped budgets", false),
		),
	...
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			...
			gomcp.StringProp("caller_id", "Filter by caller correlation id", false),
			gomcp.StringProp("since", "RFC3339 lower-bound timestamp", false),
			gomcp.NumberProp("limit", "Max rows to return (default 100, max 2000)", false),
			gomcp.BooleanProp("errors_only", "When true, only return failed events", false),
		),
	...
	a.addTool(s, gomcp.Tool{
		...
		InputSchema: gomcp.InputSchema(
			...
			gomcp.NumberProp("max_events", "Maximum matching events to return before stopping (default 1)", false),
		),

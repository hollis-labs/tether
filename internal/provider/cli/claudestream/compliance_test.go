...
// the claudestream Runtime using a sh-backed plan so no real Claude CLI is
// needed. PlanScopedAdapter (the catalog binary + base-args wrapper) is
// covered by this run since claudestream.New funnels through it.
func TestCompliance(t *testing.T) {
	...
	compliance.Run(t, compliance.Harness{
		...
		BinarySkip: true, // skip cap-tests requiring real claude binary
	})
}
